...
#'
#' This function draws a heatmap taking account marker expression levels for each identified cluster and shows the percentaje of cells for each one.
...
plot_clustering_heatmap_wrapper <- function(expr, expr_saturated, cell_clusters, cluster_merging = NULL){
  #calculate the median expression
  expr_median <- data.frame(expr, cell_clusters = cell_clusters) %>%
    dplyr::group_by(.data$cell_clusters) %>% dplyr::summarize_all(.data, stats::median(.data$cell_clusters))
  ...
  color_heat <- grDevices::colorRampPalette(RColorBrewer::brewer.pal(n = 9, name = "YlGnBu"))(100)
  ...
  pheatmap::pheatmap(expr_heat, color = color_heat, cluster_cols = FALSE,
...
#' dr.plotting
#'
#' This function plots a selected dimensional reduction from those previously calculated with FlowCT::dim.reduction().
...
dr.plotting <- function(data, dr_calculated, color_by = "expression", facet_by = NULL, labels = NULL,
                        size = 0.5, output_name = "", output_type = NULL){
  ...
  if(is.factor(data[,color_by])){
    ...
    g <- g + scale_color_gradientn(colours = grDevices::colorRampPalette(rev(RColorBrewer::brewer.pal(n = 11, name = "Spectral")))(50), name = color_by)
  }
  ...
  if(is.null(output_type)){
    ...
    ggsave(plot = g, filename = paste0(output_name, "dr.", dr_calculated, "_col.", color_by, ".", output_type), device = output_type)
  }

  return(paste0(dr_calculated, " reduction colored by ", color_by,
...
#'
#' This function plots a circular dendrogram for clusters previously calculated in order to select those nodes for coloring in \code{\link[FlowCT:circ.tree]{FlowCT::circ.tree()}}.
...
circ.tree.selectNodes <- function(expr, expr_saturated, cell_clusters, dist_method = "euclidean", hclust_method = "average"){
  ## Circular hyerarchical clustering tree
  #median expression of each marker for each cell population
  expr_medianL <- data.frame(expr, cell_clustering = cell_clusters) %>%
    dplyr::group_by(.data$cell_clustering) %>% dplyr::summarize_all(.data, stats::median(.data$cell_clustering)) %>% as.data.frame()
  expr01_medianL <- data.frame(expr_saturated, cell_clustering = cell_clusters) %>%
    dplyr::group_by(.data$cell_clustering) %>% dplyr::summarize_all(.data, stats::median(.data$cell_clustering)) %>% as.data.frame()
  ...
  dL <- stats::dist(expr_medianL, method = dist_method)
  cluster_rowsL <- stats::hclust(dL, method = hclust_method)
  ...
  ggtree::ggtree(hca, layout = "fan", branch.length = 1) +
    ggtree::geom_text2(aes_string(subset =! "isTip", label = "node"), hjust = -.3) + ggtree::geom_tiplab()
}
...
#' @param dendro_labels Logical whether adding labels to dendrogram. Default = FALSE
#' @param nodes Those nodes limiting different color areas. Default = NULL
...
circ.tree <- function(expr, expr_saturated, cell_clusters, dist_method = "euclidean", hclust_method = "average",
  ...
  p1 <- ggtree::ggtree(hca, layout = "fan", open.angle = open_angle, branch.length = 1) +
    geom_point(data = clustering_propL, aes_string(shape = "1", color = "cell_clusters", size = "Freq")) +
  ...
  if(!is.null(nodes)){
    for(i in nodes){
      p1 <- p1 + ggtree::geom_hilight(node = i, fill = sample(colors_palette, 1), alpha = .6)
    }
  }
  ...
  ggtree::gheatmap(p1, expr_tree_plot, offset = 0.5, width=1, font.size=2.5, colnames_angle=0, hjust=0,
...
cor.plot.conditions <- function(data, colname_condition, colname_patientID, conditions, metadata){
  ...
  corr <- Hmisc::rcorr(scale(dataset_reduced)) #Pearson's correlation
  ...
  corrplot::corrplot(corr_r, order = "hclust", p.mat = pval, insig = "label_sig",
           ...
           col = grDevices::colorRampPalette(rev(c("#67001F", "#B2182B", "#D6604D", "#F4A582",
...
barplot.cell.pops <- function(cell_clusters, metadata, colname_sampleID){
  ...
  print(ggplot2::ggplot(ggdfL, aes_string(x = "sample_id", y = "proportion", fill = "cell_clusters")) +
...
cell.count.bx <- function(data, counts_by = "sample_id", metadata, labels = NULL){
  ...
  g <- ggplot2::ggplot(ggdf, aes_string(x = "condition", y = "cell_counts", fill = "condition"))+
    geom_boxplot()+
  ...
  if(!is.null(labels)){
    g + ggrepel::geom_label_repel(aes(label = eval(parse(text = labels))), alpha=0.75, fontface = 'bold', color = 'black')
  }else{
    print(g)
  }
}
