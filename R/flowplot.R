...
#' @param fcs.SCE A \code{fcs.SCE} object generated through \code{\link[FlowCT:fcs.SCE]{FlowCT::fcs.SCE()}}. By default, the matrix used is the \code{arcsinh} transformed.
...
flowplot <- function(fcs.SCE, assay.i = "normalized", x.axis, y.axis, densities = T, color.by, select.values.color = "all", size = 0.5, colors = NULL){
  ...
  for(i in 1:nrow(combos)){
    ...
    if(densities){
      xdens <- cowplot::axis_canvas(g1, axis = "x")+ #histogram along x axis
        scale_fill_manual(values = colors) +
        geom_density(data = data, aes_string(x = combos[i,1], fill = color.by),
                     alpha = 0.7, size = 0.2)

      ydens <- cowplot::axis_canvas(g1, axis = "y", coord_flip = TRUE) + #histogram along y axis
        scale_fill_manual(values = colors) +
        geom_density(data = data, aes_string(x = combos[i,2], fill = color.by), alpha = 0.7, size = 0.2)+
        coord_flip()

      p1 <- cowplot::insert_xaxis_grob(g1 + theme(legend.position = "none"), xdens, unit(.2, "null"), position = "top")
      gglist[[i]] <- cowplot::insert_yaxis_grob(p1, ydens, unit(.2, "null"), position = "right")
    }else{
