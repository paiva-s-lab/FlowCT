...
#' @param rpart.control List with parameters for controling \code{rpart}. See \code{?rpart::rpart.control} for more information and defaults values (caution because, in \code{FlowCT} there are some changes: \code{cp = 0} and \code{minsplit = 10}).
...
rpart.tree <- function(fcs.SCE, cell.clusters, variables, value = "percentage", time.var, event.var, xerror, rpart.control, return.data = T){
  ...
  if(missing(rpart.control)) rpart.control <- c(list(cp = 0, minsplit = 10), formals(rpart::rpart.control)[-c(1,3)])
  
  set.seed(33)
  if(missing(time.var)){
    f <- as.formula(paste0(event.var, " ~ ", paste(unique(variables), collapse = "+")))
    tr <- rpart::rpart(f, data = surv_data, method = "class", control = rpart.control)
  }else{
    ...
    if(!is.na(as.numeric(table(surv_data[,time.var] == 0)[2]))) 
      sprintf(">>> Attention, %g value(s) were discarded because % was zero.\n", as.numeric(table(surv_data[,time.var] == 0)[2]), time.var)
    surv_data <- surv_data[surv_data[,time.var] > 0,]
    
    f <- as.formula(paste0("Surv(", time.var, ", ", event.var, ") ~ ", paste(unique(variables), collapse = "+")))
    tr <- rpart::rpart(f, data = surv_data, method = "exp", control = rpart.control)
  }
